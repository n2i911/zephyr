...
LOG_MODULE_REGISTER(modbus_s_pmm, CONFIG_MODBUS_LOG_LEVEL);
...
void modbus_reset_stats_dy(struct modbus_context *ctx)
{
...
static void update_msg_ctr_dy(struct modbus_context *ctx)
{
	ctx->mbs_msg_ctr++;
}

static void update_crcerr_ctr_dy(struct modbus_context *ctx)
{
	ctx->mbs_crc_err_ctr++;
}

static void update_excep_ctr_dy(struct modbus_context *ctx)
{
	ctx->mbs_except_ctr++;
}

static void update_server_msg_ctr_dy(struct modbus_context *ctx)
{
	ctx->mbs_server_msg_ctr++;
}

static void update_noresp_ctr_dy(struct modbus_context *ctx)
{
	ctx->mbs_noresp_ctr++;
}
#else
#define modbus_reset_stats_dy(...)
#define update_msg_ctr_dy(...)
#define update_crcerr_ctr_dy(...)
#define update_excep_ctr_dy(...)
#define update_server_msg_ctr_dy(...)
#define update_noresp_ctr_dy(...)
...
static bool mbs_fc01_coil_read(struct modbus_context *ctx)
{
	...
	bool coil_state;
	int err;
    uint8_t id = ctx->tx_adu.unit_id;
    uint8_t fc = ctx->tx_adu.fc;
	uint16_t coil_addr;
	uint16_t coil_qty;
	uint16_t num_bytes;
	uint8_t bit_mask;
	...
	if (ctx->mbs_pmm_user_cb->coil_rd == NULL) {
	...
	while (coil_cntr < coil_qty) {
		...
		err = ctx->mbs_pmm_user_cb->coil_rd(id, coil_addr, &coil_state);
		if (err != 0) {
...
static bool mbs_fc02_di_read(struct modbus_context *ctx)
{
	...
	if (ctx->mbs_pmm_user_cb->discrete_input_rd == NULL) {
		mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
		return true;
	}
	...
	while (di_cntr < di_qty) {
		...
		err = ctx->mbs_pmm_user_cb->discrete_input_rd(id, di_addr, &di_state);
		if (err != 0) {
...
static bool mbs_fc03_hreg_read(struct modbus_context *ctx)
{
	...
	uint8_t trans_id = TRANSPARENT_NOT_SUPPORT;
	/* liteon end */
	...
	if (ctx->trans_status == MODBUS_TRANS_DONE) {
		trans_mode = true; // to pass to end modbus callback
		goto resume_trans;
	}
	/* liteon end */
	...
	if ((reg_addr < MODBUS_FP_EXTENSIONS_ADDR) ||
		...
		if (ctx->mbs_pmm_user_cb->holding_reg_rd == NULL) {
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
			return true;
		}
		...
		if (ctx->mbs_pmm_user_cb->holding_reg_rd_fp == NULL) {
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
			return true;
		}
	...
	/* liteon start */
	struct modbus_pmm_id *pmm_id = _modbus_find_pmm_id(ctx, id);
	if (pmm_id != NULL) {
		trans_mode = pmm_id->trans;
		trans_id = pmm_id->trans_id;
	}

	if (trans_mode && ctx->mbs_pmm_user_cb->rtu_rd_trans_task == NULL) {
		LOG_INF("Transparent callback 'rtu_rd_trans_task' is not set");
		return false;
	}

	if (ctx->mbs_pmm_user_cb->rtu_rd_start != NULL) {
		err = ctx->mbs_pmm_user_cb->rtu_rd_start(id, trans_id, fc, start_addr, num_regs);
		if (err != 0) {
	...
	if (trans_mode) {
		err = ctx->mbs_pmm_user_cb->rtu_rd_trans_task(id, trans_id, fc, start_addr, num_regs);
		if (err != 0) {
			LOG_INF("Error handling treansparent mode");
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_DATA_ADDR);
			goto end;
		}
		ctx->trans_status = MODBUS_TRANS_WAIT_NOTIFY;
		return false;
	}
...
resume_trans:
	...
	if (trans_mode && ctx->trans_result != 0) {
		mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_DATA_ADDR);
		goto end;
	}
	/* liteon end */
	...
	while (reg_qty > 0) {
		if (reg_addr < MODBUS_FP_EXTENSIONS_ADDR) {
			...
			/* Read integer register */
			err = ctx->mbs_pmm_user_cb->holding_reg_rd(id, reg_addr, &reg);
			if (err == 0) {
			...
			/* Read floating-point register */
			err = ctx->mbs_pmm_user_cb->holding_reg_rd_fp(id, reg_addr, &fp);
			if (err == 0) {
	...
	/* liteon define start */
	if (ctx->mbs_pmm_user_cb->rtu_rd_end != NULL) {
		ctx->mbs_pmm_user_cb->rtu_rd_end(id, trans_id, fc, start_addr, num_regs);
	}
	/* liteon define end */
...
static bool mbs_fc04_inreg_read(struct modbus_context *ctx)
{
	...
	if ((reg_addr < MODBUS_FP_EXTENSIONS_ADDR) ||
	    !IS_ENABLED(CONFIG_MODBUS_FP_EXTENSIONS)) {
		/* Read integer register */
		if (ctx->mbs_pmm_user_cb->input_reg_rd == NULL) {
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
			return true;
		}
		...
		/* Read floating-point register */
		if (ctx->mbs_pmm_user_cb->input_reg_rd_fp == NULL) {
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
			return true;
		}
...
resume_trans:
	...
	while (reg_qty > 0) {
		if (reg_addr < MODBUS_FP_EXTENSIONS_ADDR) {
			...
			/* Read integer register */
			err = ctx->mbs_pmm_user_cb->input_reg_rd(id, reg_addr, &reg);
			if (err == 0) {
				sys_put_be16(reg, presp);
				presp += sizeof(uint16_t);
			}
			...
			/* Read floating-point register */
			err = ctx->mbs_pmm_user_cb->input_reg_rd_fp(id, reg_addr, &fp);
			if (err == 0) {
				memcpy(&reg, &fp, sizeof(reg));
				sys_put_be32(reg, presp);
				presp += sizeof(uint32_t);
...
static bool mbs_fc05_coil_write(struct modbus_context *ctx)
{
	...
	if (ctx->mbs_pmm_user_cb->coil_wr == NULL) {
		mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
		return true;
	}
	...
	err = ctx->mbs_pmm_user_cb->coil_wr(id, coil_addr, coil_state);
...
static bool mbs_fc06_hreg_write(struct modbus_context *ctx)
{
	...
	if (ctx->mbs_pmm_user_cb->holding_reg_wr == NULL) {
		mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
		return true;
	}
	...
	err = ctx->mbs_pmm_user_cb->holding_reg_wr(id, reg_addr, reg_val);
...
static bool mbs_fc15_coils_write(struct modbus_context *ctx)
{
	...
	while (coil_cntr < coil_qty) {
		...
		err = ctx->mbs_pmm_user_cb->coil_wr(id, coil_addr + coil_cntr,
						coil_state);
...
static bool mbs_fc16_hregs_write(struct modbus_context *ctx)
{
	...
	if ((reg_addr < MODBUS_FP_EXTENSIONS_ADDR) ||
		...
		if (ctx->mbs_pmm_user_cb->holding_reg_wr_fp == NULL) {
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_FC);
			return true;
		}
	...
	if (trans_mode && ctx->mbs_pmm_user_cb->rtu_wr_trans_task == NULL) {
		LOG_INF("Transparent callback 'rtu_wr_trans_task' is not set");
		return false;
	}

	if (ctx->mbs_pmm_user_cb->rtu_wr_start != NULL) {
		err = ctx->mbs_pmm_user_cb->rtu_wr_start(id, trans_id, fc, start_addr, num_regs);
		if (err != 0) {
			LOG_INF("Holding register address not supported");
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_DATA_ADDR);
			goto end;
	...
	for (uint16_t reg_cntr = 0; reg_cntr < reg_qty; reg_cntr++) {
		...
		if ((reg_addr < MODBUS_FP_EXTENSIONS_ADDR) ||
			...
			err = ctx->mbs_pmm_user_cb->holding_reg_wr(id, addr, reg_val);
		} else {
			...
			err = ctx->mbs_pmm_user_cb->holding_reg_wr_fp(id, addr, fp);
		}
	...
	/* inturrupt point for waiting transparent operation */
	if (trans_mode) {
		err = ctx->mbs_pmm_user_cb->rtu_wr_trans_task(id, trans_id, fc, start_addr, num_regs);
		if (err != 0) {
			LOG_INF("Error handling treansparent mode");
			mbs_exception_rsp(ctx, MODBUS_EXC_ILLEGAL_DATA_ADDR);
			goto end;
...
resume_trans:
	...
	if (ctx->mbs_pmm_user_cb->rtu_wr_end != NULL) {
		ctx->mbs_pmm_user_cb->rtu_wr_end(id, trans_id, fc, start_addr, num_regs);
	}
	/* liteon define end */
...
bool modbus_server_handler_pmm(struct modbus_context *ctx)
{
	...
	/* liteon start */
	if (ctx->trans_status == MODBUS_TRANS_TIMEOUT) {
		LOG_ERR("Transparent timeout");
	...
	update_msg_ctr_dy(ctx);

	if (ctx->rx_adu_err != 0) {
		update_noresp_ctr_dy(ctx);
		if (ctx->rx_adu_err == -EIO) {
			update_crcerr_ctr_dy(ctx);
		}
	...
	if (ctx->pmm_server) {
		...
		SYS_SLIST_FOR_EACH_NODE(&ctx->pmm_ids, snp) {
			struct modbus_pmm_id *pmm_id =
				CONTAINER_OF(snp, struct modbus_pmm_id, node);
		...
		if (addr != 0 && !id_found) {
			LOG_DBG("Unit ID %u doesn't match", addr);
			update_noresp_ctr_dy(ctx);
			return false;
		}
	}
